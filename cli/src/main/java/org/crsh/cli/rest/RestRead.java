...
public class RestRead {
  ...
  static public String buildQueryParams(Properties pros) {
    String params = "";
    if (pros != null) {
      Enumeration<Object> enumeration = pros.elements();
      while (enumeration.hasMoreElements()) {
        ...
        params += s + "=" + pros.getProperty(s).replace(" ", "%20").replace("&", "%26");
      }
    }
    return params;
  }
  
  static public Properties convertQueryParams(String params) {
    Properties pros = new Properties();
    if(params != null && params.length() > 0) {
      try {
        pros = ValueType.PROPERTIES.parse(params);
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
