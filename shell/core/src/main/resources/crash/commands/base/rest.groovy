...
public class rest extends org.crsh.command.CRaSHCommand {
  ...
  Command use for new activity:
  % rest newactivity fetch {activityId} - get the activity
  % rest newactivity rm {activityId} - remove activity
  % rest newactivity like {activityId} like - like/unlike activity
  % rest newactivity comments {activityId} comments => return all 
  % rest newactivity comments {activityId}  limit=10;offset=0 => return list 
  % rest newactivity add {activityId}  text='the title' - post new a comment
  % rest newactivity update {activityId} {commentId} title='the new title' - update a comment
  % rest newactivity rm {activityId}  {commentId} - remove the comment

  Command use for new Stream activities:
  % rest newstream user {user's Id} - get the activities stream of user
  % rest newstream user {user's Id} type=connections;after=xxx;before=yyy - get the activities stream of user with query params
  % rest newstream space {space's Id} - get the activities stream of space

  Command use for new User:
  % rest newuser {user's Id} - get the information of user
  % rest newuser {user's Id} c - get the connections of user
  % rest newuser q=xx* - find users by input query param
  ...
  @Man("The syntax: rest login -u {user} -p {pass} ")
  @Command
  public Object login(
  ...
  @Usage("Providing server configuration.")
  @Man("The syntax: domain=http://localhost:8080;rest=rest;")
  @Command
  public void use(
    @Argument(unquote = false)
    @Usage("""\

    domain: The domain of rest service (by default is domain=http://localhost:8080)
    ...
    """)
    @Man("Input the information of rest system") java.util.Properties parameters) {
    ...
    portalName = "portal";
    try {
			portalName = org.exoplatform.container.PortalContainer.getCurrentPortalContainerName();
    } catch (Exception e) {}
    //
    ...
    //
		...
		sysProps.put("rmactivity", "method=POST;path=" + portalName + "/social/activities/destroy/{activityId}.json");
		sysProps.put("likesactivity", "method=GET;path=" + portalName + "/social/activities/{activityId}/likes/show.json");
		sysProps.put("likeactivity", "method=POST;path=" + portalName + "/social/activities/{activityId}/likes/update.json");
		sysProps.put("unlikeactivity", "method=POST;path=" + portalName + "/social/activities/{activityId}/likes/destroy/{identityId}.json");
		sysProps.put("commentsactivity", "method=GET;path=" + portalName + "/social/activities/{activityId}/comments/show.json");
		sysProps.put("commentslimitactivity", "method=GET;path=" + portalName + "/social/activities/{activityId}/comments.json");
		sysProps.put("fetchactivity", "method=GET;path=" + portalName + "/social/activities/{activityId}.json");
		sysProps.put("updatecommentactivity", "method=POST;path=" + portalName + "/social/activities/{activityId}/comments/update.json");
		sysProps.put("addcommentactivity", "method=GET;path=" + portalName + "/social/activities/{activityId}/comments/create.json");
		sysProps.put("rmcommentactivity", "method=POST;path=" + portalName + "/social/activities/{activityId}/comments/destroy/{commentId}.json");
  }
  ...
  public Object activity (
    ...
    def params = (parameters != null) ? RestRead.buildQueryParams(parameters) : "";
    if(params.equals("") && commentId != null && commentId.indexOf("=") > 0) {
      params = commentId;
    }
    
    def key = action.trim().toLowerCase();
    if("rm".equalsIgnoreCase(key) && notNull(commentId)) {
			key = "rmcomment";
		} else if("update".equalsIgnoreCase(key)) {
			if(notNull(commentId)) {
				key = "updatecomment";
			} else {
				throw new ScriptException("Need input the comment's Id for update comment.");
			}
		} else if("add".equalsIgnoreCase(key)) {
			key = "addcomment";
...
//===============================================================================================//  
  ...
  public Object newuser (
    ...
    if(parameters == null) {
      parameters = RestRead.convertQueryParams(param);
    }
    if(parameters != null && isQuery && 
       (parameters.get("q") == null || parameters.get("q").length() == 0)){
      throw new ScriptException("Need input the user's Id or query param by syntax: q=xxx");
    }
    //
    return get(path, parameters);
  }
